...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleAuth = async (e: React.FormEvent) => {
        ...
        try {
            if (isSignUp) {
                // ✅ Check if profile is actually selected (redundant security)
                if (!profileSelected) {
                ...
                setIsSignUp(false);
                setProfileSelected(false); // Reset
            } else {
    ...
    const toggleMode = () => {
        ...
        setProfileSelected(false); // Reset selection logic when switching modes
    };
    ...
    return ReactDOM.createPortal(
        <div className="fixed inset-0 z-[10001] flex items-center justify-center bg-black/60 backdrop-blur-sm overflow-y-auto py-10">
            <div className={`bg-slate-900 rounded-3xl shadow-2xl w-full p-8 relative border border-slate-700 my-auto transition-all duration-300 ${isSignUp && profileSelected ? 'max-w-2xl' : 'max-w-md'}`}>
                <button
                    onClick={onClose}
                ...
                <form className="space-y-4" onSubmit={handleAuth}>
                    {isSignUp && (
                        <>
                            {/* Profile Type Toggle - ALWAYS VISIBLE IN SIGNUP */}
                            ...
                            {/* CONDITIONAL RENDER: Form Fields */}
                            {profileSelected && (
                                    ...
                                    <>
                                        <div className="flex items-center mt-4">
                                            ...
                                            <label htmlFor="terms" className="ml-2 text-sm font-medium text-gray-300">
                                                ...
                                                <button type="button" onClick={() => openLegalModal('privacy')} className="text-primary-600 hover:underline text-primary-500 font-semibold">POLÍTICA DE PRIVACIDADE</button> e{' '}
