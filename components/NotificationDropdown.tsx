...
export const NotificationDropdown: React.FC<NotificationDropdownProps> = ({ notifications, onClose, onMarkAsRead }) => {
  ...
  return (
    <div className="absolute right-0 top-12 w-80 md:w-96 bg-slate-800 rounded-3xl shadow-2xl border border-slate-700 z-50 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden z-index-999">
        ...
        <div className="flex items-center space-x-2 z-index-999">
          {unreadCount > 0 && (
            <button
              ...
              className="text-xs text-primary-500 font-medium hover:text-primary-600 hover:text-primary-400 transition-colors"
            >
        ...
        {notifications.length > 0 ? (
          <div className="divide-y divide-gray-100 divide-slate-700">
            {notifications.map((notif) => (
              <div
                key={notif.id}
                className={`group relative p-4 hover:bg-slate-750 transition-colors ${!notif.lida ? 'bg-blue-50/50 bg-blue-900/10' : ''} ${deletingId === notif.id ? 'opacity-50' : ''}`}
              >
                <div
                ...
                <button
                  ...
                  className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-1.5 rounded-full bg-red-50 bg-red-900/20 text-red-600 text-red-400 hover:bg-red-100 hover:bg-red-900/40 transition-all"
      ...
      {notifications.length > 0 && (
        <div className="p-3 border-t border-slate-700 text-center bg-slate-900/50">
          <button
            onClick={() => {
              navigate('/notifications');
              onClose();
            }}
            className="text-xs font-medium text-gray-300 hover:text-primary-500 hover:text-primary-400 transition-colors"
          >
