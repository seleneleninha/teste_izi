...
import { Building2, MapPin, Building, MapPinHouse, MapPinned, Tractor, Map as MapIcon } from 'lucide-react';
...
const PropertyMap = dynamic(() => import('@/components/PropertyMap'), {
  ssr: false,
  loading: () => <div className="h-[400px] w-full bg-gray-100 flex items-center justify-center">Carregando mapa...</div>
});
...
export default function Home() {
  ...
  return (
    <main className="bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-white font-sans transition-colors duration-200 min-h-screen flex flex-col">
      {/* Hero Section */}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {[
              ...
              <div
                key={idx}
                onClick={() => router.push(`/search?tipo=${category.type}`)}
              ...
              {cities.map((city, idx) => (
                <div key={idx} className="flex-none w-64" style={{ scrollSnapAlign: 'start' }}>
                  <div
                    ...
                    onClick={() => router.push(`/search?cidade=${encodeURIComponent(city)}`)}
                  >
              ...
              {neighborhoods.map((neighborhood, idx) => (
                <div key={idx} className="flex-none w-64" style={{ scrollSnapAlign: 'start' }}>
                  <div
                    ...
                    onClick={() => router.push(`/search?bairro=${encodeURIComponent(neighborhood)}`)}
                  >
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end">
