            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
                    {configs.map((config) => {
                        ...
                        return (
                            <div key={config.key} className="bg-slate-800 rounded-2xl p-4 md:p-6 shadow-sm border border-slate-700 relative group hover:border-emerald-500/30 transition-all">
                                {/* Header with icon and key */}
                                <div className="flex items-center justify-between mb-3">
                                    <div className={`p-2.5 md:p-3 rounded-xl ${bgColor} ${color}`}>
                                        <Icon size={20} className="md:w-6 md:h-6" />
                                    </div>
                                    <div className="bg-slate-900 rounded-lg px-2 py-1 text-[10px] md:text-xs font-mono text-slate-500 truncate max-w-[120px]">
                                        {config.key}
                                    </div>
                                </div>

                                {/* Label and description */}
                                <h3 className="text-base md:text-lg font-bold text-white mb-1">{label}</h3>
                                <p className="text-xs md:text-sm text-slate-400 mb-4 line-clamp-2">{config.description}</p>
                                ...
                                <div className="flex items-center gap-2 bg-slate-900/50 p-2 rounded-xl border border-slate-800">
                                    <input
                                        ...
                                        className="flex-1 min-w-0 bg-transparent border-none focus:ring-0 text-white font-bold text-lg md:text-xl text-center outline-none"
                                    />
                                    <button
                                        onClick={() => handleSave(config.key, config.value)}
                                        className="p-2.5 md:p-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl shadow-lg shadow-emerald-500/20 transition-all hover:scale-105 shrink-0"
